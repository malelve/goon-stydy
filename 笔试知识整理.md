...
**至于为什么是8**：泊松分布显示，hash冲突导致链表长度超过8的可能非常小
...
jdk1.8后的TreeMap是基于红黑树实现的，HashMap当链表长度大于8时，剩下的数据也会存入红黑树
...
堆常用于堆排序，利用大根堆和小根堆，在海量数据中排序或寻找前几位效率最优，时间复杂度为O(n log n)空间复杂度为O(1)
...
https://blog.csdn.net/weixin_41832850/article/details/106065055
...
HTTP 协议中规定 POST 提交的**数据必须在 body 部分中**，但是协议中没有规定数据使用哪种**编码方式**或者**数据格式**。实际上，开发者完全可以自己决定消息主体的格式，只要最后发送的 HTTP 请求满足上面的格式就可以。
...
这种方式**一般用来上传文件**，各大服务端语言对它也有着良好的支持。
...
- HTTP 是一个无状态协议，这意味着每个请求都是独立的，Keep-Alive 没能改变这个结果。另外，Keep-Alive也不能保证客户端和服务器之间的连接一定是活跃的，在 HTTP1.1 版本中也如此。**唯一能保证的就是当连接被关闭时你能得到一个通知**，所以不应该让程序依赖于 Keep-Alive 的保持连接特性，否则会有意想不到的后果。
  - 使用**长连接**之后，客户端、服务端怎么知道本次传输结束呢？
  - 两部分：
    - 1. 判断传输数据是否达到了Content-Length 指示的大小；
      2.  动态生成的文件没有 Content-Length ，它是分块传输（chunked），这时候就要根据 chunked 编码来判断，chunked 编码的数据在最后有一个空 chunked 块，表明本次传输数据结束，详见[这里](http://www.cnblogs.com/skynet/archive/2010/12/11/1903347.html)。什么是 chunked 分块传输呢？下面我们就来介绍一下。
...
## [HTTP over SSL/TLS (HTTPS)](https://www.bilibili.com/video/BV1M44y1175D/?spm_id_from=333.337.search-card.all.click&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)
...
1. 对称加密：客户端和服务端都持有相同的密钥，这个密钥是一对一的，不对外公开，双方利用私钥进行加密和解密
2. 非对称加密：服务端生成一个私钥，一个公钥，公钥公开，由任意客户端获取，并加密内容，私钥只有服务端一方持有，用于解密



首先，我们思考对称加密的问题：

- 因为对称加密，密钥是一对一的，那么如果有100个客户端，那么服务器不得存100个私钥吗，这也太浪费了吧



然后，我们再看非对称加密的问题：

- 现在因为是公钥公开，只需要一个私钥，那么已经解决了对称加密的一对一问题，现在是一对多，但是呢，非对称加密太消耗性能了，而且加密的内容越多，性能还线性下降！！！不可取



所以，现在我们的问题是，即想要对称加密的高性能，又想要非对称加密的多对一，那么我们该怎么办呢？

- 答案是，将两者结合：首先我们利用非对称加密，客户端利用公钥，加密一个RendKey并发送给服务器，服务器利用私钥进行解密，现在客户端和服务端都得到了一个共同的RendKey，这不就是对称加密我们想要的私钥吗！！！然后剩下的过程，客户端和服务端都利用这个私钥进行加密解密，这部就完美了吗！！！



但是，世界上没有不透风的墙，现在我们遇到的问题是，如果有中间人拦截双方数据，然后冒充双方，分别对双方发送中间人自己假冒的公钥怎么办，这不就是中间人拦截服务器，并向客户端假冒服务器吗？

- 答案是，引入CA认证技术，即数字认证技术：公钥和私钥不由服务器颁发，而是交给一个大家都信任的机构颁发，客户端都持有信任的机构，服务端也向机构进行注册，并获取机构给的公钥和私钥，再交给客户端，客户端拿到公钥后，通过自己持有的信任的机构进行验证，确认是机构颁发的才进行后续请求，这部就迎刃而解了吗！！！

通过以上过程，我们可以看到，HTTPS中，**对称加密算法**、**密钥交互算法**和**消息认证算法**的原理。
...
- 什么是 SYN 攻击（SYN Flood）？
  ...
  在**三次握手过程**中，服务器发送 SYN-ACK 之后，收到客户端的 ACK 之前的 TCP 连接称为半连接(half-open connect)。此时服务器处于 SYN_RCVD 状态。当收到 ACK 后，服务器才能转入 ESTABLISHED 状态.
...
<img src="https://s2.51cto.com/images/blog/202108/02/8516c556e896d57073cedc0d157012f2.png?x-oss-process=image/watermark,size_16,text_QDUxQ1RP5Y2a5a6i,color_FFFFFF,t_30,g_se,x_10,y_10,shadow_20,type_ZmFuZ3poZW5naGVpdGk=/format,webp/resize,m_fixed,w_1184" alt="快问快答，计算机网络面试夺命20问_服务端_03" style="zoom: 67%;" />
...
<img src="https://s2.51cto.com/images/blog/202108/02/0ef421fb820ff26e645a073f8781a92c.png?x-oss-process=image/watermark,size_16,text_QDUxQ1RP5Y2a5a6i,color_FFFFFF,t_30,g_se,x_10,y_10,shadow_20,type_ZmFuZ3poZW5naGVpdGk=/format,webp/resize,m_fixed,w_1184" alt="快问快答，计算机网络面试夺命20问_服务端_06" style="zoom:67%;" />
...
对此要理解的一个是，在系统中内核并不是作为一个与用户进程平行的进程的集合，**内核是为用户进程运行**的。
...
- 高级调度：(High-Level Scheduling)又称为作业调度，它决定把后备作业**调入内存**运行；
- 中级调度：(Intermediate-Level Scheduling)又称为在**虚拟存储器中引入**，在**内、外存对换区进行进程对换**。
...
1. 轻型实体线程中的实体基本上不拥有系统资源，只是有一点必不可少的、能保证独立运行的资源。**线程的实体包括程序、数据和TCB**。线程是动态概念，它的动态特性由**线程控制块TCB**（Thread Control Block）描述。
...
4. 共享进程资源。 在**同一进程中的各个线程，都可以共享该进程所拥有的资源**，这首先表现在：所有**线程都具有相同的地址空间（进程的地址空间）**，这意味着，线程可以访问该地址空间的每一个虚地址；此外，还可以访问进程所拥有的已打开文件、定时器、信号量等。由于同一个进程内的线程共享内存和文件，所以**线程之间互相通信不必调用内核（进程间的通信是需要调用内核的）**。 线程共享的环境包括：**进程代码段、进程的公有数据(利用这些共享的数据，线程很容易的实现相互之间的通讯**)、进程打开的文件描述符、信号的处理器、进程的当前目录和进程用户ID与进程组ID。
...
- 协程：协程的调度完全**由用户控制**（也就是在**用户态执行**）。协程拥有**自己的寄存器上下文和栈**。协程调度切换时，将寄存器上下文和栈**保存到线程的堆区**，在切回来的时候，恢复先前保存的寄存器上下文和栈，直接操作栈则基本没有内核切换的开销，可以不加锁的访问全局变量，所以上下文的切换非常快。（**最主要的区别时用户自己调度**）

- 协程最大的优势就是协程极高的执行效率。因为**子程序切换不是线程切换，而是由程序自身控制，因此，没有线程切换的开销**，和线程切换相比，线程数量越多，协程的性能优势就越明显。不需要多线程的锁机制，因为只有一个线程，也不存在同时写变量冲突，在协程中控制共享资源不加锁，只需要判断状态就好了，所以执行效率比多线程高很多。此外，一个线程的内存在MB级别，而协程只需要KB级别。
...
- 用**过去的历史预测将来**，选最近最长时间没有使用的页淘汰(也称最近最少使用)。
...
- 如果它的**访问位**位是 0 就淘汰该页面，并把新的页面插入这个位置，然后把表针前移一个位置；
...
## [CPU](https://www.cnblogs.com/cxuanBlog/p/11712964.html)
...
例如A表20条记录，B表30条记录，两者关联后笛卡尔积共20*30 = 600条纪律，即A表和B表的**每条数据都关联了一遍**。我们常用的**左连接、右连接、内连接就是避免笛卡尔积**的方式。
...
​	左表数据全部都在 + 右表的匹配的数据，左表中的数据右表无匹配，则字段为NULL，

### 右连接

​	`RIGHT JOIN` / `RIGHT OUT JOIN`

​	右表的数据全部都在 + 坐标的匹配的数据，右表中的数据坐标无匹配，则字段为NULL
...
- 支持事务
- 支持行锁（通过索引上锁实现，**`只有通过索引条件检索数据，InnoDB才会使用行级锁，否则，InnoDB将使用表锁`！**）、表锁，因此可以支持并发
- 不存储总行数
- 索引和数据在一个文件
...
```python
# map函数，依次映射第一个函数和第二个序列，python2.X返回列表，python3.x返回迭代器
# 求最大值
dp = [[1,2],[2,3],[3,4]]
max(map(max, dp))
# >> 4
...
运行时数据：存储对象自身的运行数据，如**哈希码**，**GC代年龄**，**锁状态**、**线程持有的锁**、**偏向线程ID**等。
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/7/17/164a85bd83298d96~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
有且只有这一种情况，eden满了，触发gc，又称为MinoGC
...
### [NIO](https://zhuanlan.zhihu.com/p/127911769)

同步**非阻塞**

NIO是一种基于通道和缓冲区的I/O方式，NIO有三大核心部分：Channel(通道)，Buffer(缓冲区)，Selector（选择器）

客户端发送的请求不会被阻塞，而是直接进入，最多排队等待一段时间，服务器通过一个**线程轮询大量的请求channel**，每次获取一批有事件的channel，并对每个请求启动线程池中的一个线程进行处理。其核心就是非阻塞，一个客户端并不是时时刻刻都有数据进行交互，没有必要死耗着一个线程不放，所以客户端选择了让线程歇一歇，只有客户端有相应的操作的时候才发起通知，创建一个线程来处理请求。（**理解：相当于每个客户端都有一个channel，服务器从从有事件channel中获取请求，并从线程池中启动线程来进行操作**）
...
普通的IO都是用户态切换到内核态，内核态将数据读取出来到ReadBuffer，再复制到进程的私有地址空间，再写入到内核态WriteBuffer，完成读取和写入，这样是进行了两次IO操作；

而零拷贝使得ReadBuffer里的数据直接到WriteBuffer里，减少了IO操作。

java.nio里的transferTo，transfFrom就是零拷贝的一个实现。
...
通过**javap -c 反编译**.calss文件，我们可以发现Sychronized反编译会生成：monitorenter 和 monitorexit两个字节码指令。monitorenter和monitorexit将我们的核心代码进行了包裹。
...
## [Redis数据结构的底层实现](https://blog.csdn.net/a745233700/article/details/113449889)

### 两层数据结构

从**外部**调用接口角度来看，redis包括如下5个数据结构：
...
从**实现**角度看，redis又包含如下6个更底层的实现：

- **dict** ——————> 字典
- **sds** ——————> 动态字符串
- **ziplist** ——————> 特殊编码的双向链表，可以用于存储字符串或整数，其中整数是按真正的二进制表示进行编码的，而不是编码成字符串序列。（ziplist使用了一整块连续的内存，将表中每一项存放在前后连续的地址空间内，类似于一个数组）
- **quicklist** ——————> quicklist是一个基于ziplist的双向链表，quicklist的每个节点都是一个ziplist
- **skiplist** ——————> 跳表是一种可以进行二分查找的有序链表，采用空间换时间的设计思路，跳表在原有的有序链表上面增加了多级索引（例如每两个节点就提取一个节点到上一级），通过索引来实现快速查找。跳表是一种动态数据结构，支持快速的插入、删除、查找操作，时间复杂度都为O(logn)，空间复杂度为 O(n)。跳表非常灵活，可以通过改变索引构建策略，有效平衡执行效率和内存消耗
- **intset** ——————>  intset是一个由整数组成的有序集合，从而便于进行二分查找，用于快速地判断一个元素是否属于这个集合。它在内存分配上与[ziplist](http://zhangtielei.com/posts/blog-redis-ziplist.html)有些类似，是连续的一整块内存空间，而且对于大整数和小整数（按绝对值）采取了不同的编码，尽量对内存的使用进行了优化。



以下我们将通过第二层数据结构来理解redis如何逐个实现第一层数据结构的。
...
​	zset就是一个字典+[跳表](https://www.jianshu.com/p/9d8296562806)（skipList）

​	字典用于查询数据到分数，跳表用于查询分数到数据（可能是范围查找）

​	跳表其实就是**二分查找**，时间复杂度O(log n)，空间复杂度O(n)，
...
那么问题来了，既然是二分查找，为什么redis不采用红黑树呢？

​	这里我们需要思考，redis的sorted set有哪些功能？
...
**（补，最终一致性方案）借助第三方，在redis更新失败的时候，自旋更新；或者监控MySQL日志，持续更新redis**）
...
分布式锁其实就是，控制分布式系统**不同进程共同访问共享资**源的一种锁的实现。如果不同的系统或同一个系统的不同主机之间共享了某个临界资源，往往需要互斥来防止彼此干扰，以保证一致性。

<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/42884a1613344c11be5fef3b9e8ed7c5~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image" alt="img" style="zoom:50%;" />

### [redis分布式锁有哪些实现？](https://learn.lianglianglee.com/%E4%B8%93%E6%A0%8F/%E5%88%86%E5%B8%83%E5%BC%8F%E6%8A%80%E6%9C%AF%E5%8E%9F%E7%90%86%E4%B8%8E%E5%AE%9E%E6%88%9845%E8%AE%B2-%E5%AE%8C/11%20%E5%88%86%E5%B8%83%E5%BC%8F%E9%94%81%E6%9C%89%E5%93%AA%E4%BA%9B%E5%BA%94%E7%94%A8%E5%9C%BA%E6%99%AF%E5%92%8C%E5%AE%9E%E7%8E%B0%EF%BC%9F.md)

视频参考链接：https://www.bilibili.com/video/BV1wW4y177WN/?spm_id_from=333.337.search-card.all.click&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b

#### 基于数据库的分布式锁

基于关系型数据库实现分布式锁，是**依赖数据库的唯一性来实现资源锁定，比如主键和唯一索引**等。
...
常见的开源缓存组件都支持分布式锁，包括 Redis、Memcached 及 Tair。以常见的 Redis 为例，应用 Redis 实现分布式锁，最直接的想法是**利用 setnx 和 expire 命令**实现加锁。
...
扩展：setnx+expire不是一个原子操作，可以使用SET的扩展命令（SET EX PX NX）；
...
除了Redis外，Zookeeper也可以实现分布式锁

[参考连接](https://learn.lianglianglee.com/%E4%B8%93%E6%A0%8F/%E5%88%86%E5%B8%83%E5%BC%8F%E6%8A%80%E6%9C%AF%E5%8E%9F%E7%90%86%E4%B8%8E%E5%AE%9E%E6%88%9845%E8%AE%B2-%E5%AE%8C/11%20%E5%88%86%E5%B8%83%E5%BC%8F%E9%94%81%E6%9C%89%E5%93%AA%E4%BA%9B%E5%BA%94%E7%94%A8%E5%9C%BA%E6%99%AF%E5%92%8C%E5%AE%9E%E7%8E%B0%EF%BC%9F.md)
...
- 设计监视，使key快失效时及时恢复
- 使key不失效
- key失效后阻塞，只允许一个线程读取数据库



## 缓存击穿

类锁于缓存雪崩，但是是只有一个key失效，而大量并发访问同时访问这个key，然后直接进入数据库
...
## [Synchronized](https://www.jianshu.com/p/3cfdf32bd37e)

同步锁（重量级锁）

主要修饰：普通方法（对象锁）、静态方法（类锁）、代码块（可类锁：`synchronized(XX.class){}`、对象锁：`synchronized(this){}`）
...
```Java
// lettcode 1114：https://leetcode.cn/problems/print-in-order/
// 同步原语解法
class Foo {
...
## [Semaphore](https://www.cnblogs.com/Honglixi/p/15574381.html)
...
值得注意的是初始化Semaphore时，设置的许可证数量为0：`Semaphore semaphore = new Semaphore(0)`，之所以这么做是因为，信号量的许可证 `<=0` 时才会使当前线程阻塞，即当 `Semaphore(0)` 时，只要执行了 `semaphore.acquire()` 当前线程就会阻塞，必须先执行 `semaphore.release()` ,使得许可证+1才能使线程不会阻塞。
...
## [ReentranLock](https://www.liaoxuefeng.com/wiki/1252599548343744/1306580960149538)

可重入锁

tryLock(): 尝试加锁，可能加到，也可能加不到，当加到时会返回true，加不到会返回false

lock():表示阻塞加锁，加不到锁就阻塞线程，直到加到锁，也不会有返回值

公平锁：任何线程来了，只要有线程在排队（AQS），就必须排队，没有排队，则可以竞争锁

非公平锁：线程来了，先尝试竞争锁，竞争失败了就排队，成功就获取锁

配合Condition可实现Synchronized的wait()、notify()、notifyAll()
...
(1) synchronized 是**Java的一个内置关键字**，而**ReentrantLock是Java的一个类**。

(2) synchronized只能是**非公平锁**。而ReentrantLock**可以实现公平锁和非公平锁两种**。

(3) synchronized不能**[中断](https://so.csdn.net/so/search?q=中断&spm=1001.2101.3001.7020)一个等待锁的线程**，而**Lock可以中断一个试图获取锁的线程**。

(4) synchronized**不能设置超时**，而**Lock可以设置超时**。

(5) synchronized**会自动释放锁**，而**ReentrantLock不会自动释放锁，必须手动释放**，否则可能会导致死锁。
...
long stamp = stampedLock.tryOptimisticRead(); // 先获取乐观锁
...
任务拆分，分配给多个线程完成并合并，适合cpu多核情况下的提高效率





## [ThreadLocal](https://www.liaoxuefeng.com/wiki/1252599548343744/1306581251653666)

可以在一个线程内传递状态。（通过泛型）

如果线程池使用ThreadLocal不当会造成**OOM：内存泄漏**

（因为ThreadLocal的底层原理是相当于，有一个大的ThreadLocal，我们通过线程作为key，可以获取到一个线程的ThreadLocalMap，而线程的的一个ThreadLocal对象就是一个键，由此获取到值，但是我们不手动清理`threadLocal.remove()`，因为线程是线程池中的对象，强制引用Entry对象，两者都不会被回收，所以我们要**手动清除threadLocal的键值对**，同时也可以**避免这次存入的键值对影响下次**）

```java
static ThreadLocal<User> threadLocalUser = new ThreadLocal<>(); // 这里的User就是状态

// 线程内其他地方直接用
threadLocalUser.set(user); // 设置状态
User u = threadLocalUser.get(); // 获取状态
threadLocalUser.remove(); // 解除状态（一定要写，因为线程有可能回归线程池复用！！！，建议写在finally里，或者实现一个继承AutoCloseable的类，重写close方法，来自动解除状态）
```



## [协程](https://www.liaoxuefeng.com/wiki/1252599548343744/1501390373388322)

又称虚拟线程、轻量级线程、纤程、绿色线程、用户态线程
...
特点：一但IO就立刻挂起，异步等待数据返回。**由程序员控制调度**
...
<img src="https://segmentfault.com/img/bVbAcxy?w=535&h=457" alt="330611-20171023144517066-24770749.png" style="zoom:50%;" />

![12323.png](https://segmentfault.com/img/bVbAcxY?w=819&h=460)

## 过滤器filter

过滤器作用于进入容器后（tomcat）和servelt前，所以过滤器的doFilter(ServletRequest request, ServletResponse response, FilterChain chain)的入参是ServletRequest ，而不是httpservletrequest。因为过滤器是在httpservlet之前。



## 拦截器[interceptor](https://www.liaoxuefeng.com/wiki/1252599548343744/1347180610715681)

拦截器作用于Controller，可实现方法前执行、方法后执行等，可多次调用
...
　　①**拦截器是基于java的反射机制的**，而**过滤器是基于函数回调**。
　　②**拦截器不依赖与servlet容器**，**过滤器依赖与servlet容器**。
　　③**拦截器只能对action请求起作用**，而**过滤器则可以对几乎所有的请求起作用**。
　　④**拦截器可以访问action上下文、值栈里的对象**，而**过滤器不能访问**。
　　⑤在action的生命周期中，**拦截器可以多次被调用**，而**过滤器只能在容器初始化时被调用一次**。
　　⑥**拦截器可以获取IOC容器中的各个bean**，而**过滤器就不行**，这点很重要，在拦截器里注入一个service，可以调用业务逻辑。
...
属性可以使用`default`关键字来设置默认值



## [注解的使用](https://www.liaoxuefeng.com/wiki/1252599548343744/1265102026065728)
...
**方法一**是先判断`Annotation`是否存在，如果存在，就直接读取：
...
**方法二**是直接读取`Annotation`，如果`Annotation`不存在，将返回`null`：
...
**事务**可以理解为“**要么什么都不做，要么就做全套**”的机制。例如数据库的**ACID**。
...
- **CAP理论的定义** 在一个分布式系统（指互相连接并共享数据的节点的集合）中，当涉及读写操作时，只能保证一致性（Consistence）、可用性（Availability）、分区容错性（PartitionTolerance）三者中的两个，另外一个必须被牺牲。

<img src="https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/12/10/1679817cdb4305db~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp" alt="CAP" style="zoom:50%;" />
...
> The system will continue to function when network partitions occur. 当出现**网络分区**后，系统能够继续“履行职责”。
...
- CP - Consistency +  Partition Tolerance （一致性 + 分区容忍性）
  ...
  <img src="C:\Users\pc\Desktop\知识累积\CP.webp" style="zoom:50%;" />

如上图所示，因为Node1节点和Node2节点连接中断导致分区现象，Node1节点的数据已经更新到y，但是Node1 和 Node2 之间的复制通道中断，数据 y 无法同步到 Node2，Node2 节点上的数据还是旧数据x。

这时客户端C 访问 Node2 时，Node2 需要返回 Error，提示客户端 “系统现在发生了错误”，这种处理方式违 背了可用性（Availability）的要求，因此 CAP 三者只能满足 CP。

- AP - Availability +  Partition Tolerance （可用性 + 分区容忍性）

  <img src="C:\Users\pc\Desktop\知识累积\AP.webp" alt="AP" style="zoom:50%;" />
...
值得补充的是，CAP理论告诉我们**分布式系统只能选择AP或者CP**，但实际上并不是说整个系统只能选择AP或者CP，在 CAP 理论落地实践时，我们需要将系统内的数据按照不同的应用场景和要求进行分类，每类数据选择不同的策略（CP 还是 AP），而不是直接限定整个系统所有数据都是同一策略。
...
BASE理论是CAP理论的延申，是指**基本可用**（Basically Available）、**软状态**（ Soft State）、**最终一致性**（ Eventual Consistency），**核心思想是即使无法做到强一致性（CAP 的一致性就是强一致性），但应用可以采用适合的方式达到最终一致性。**

<img src="C:\Users\pc\Desktop\知识累积\BASE.webp" alt="BASE" style="zoom:50%;" />
...
- AP 方案中牺牲一致性只是指发生分区故障期间，而不是永远放弃一致性。 这一点其实就是 BASE 理论延伸的地方，分区期间牺牲一致性，但**分区故障恢复后，系统应该达到最终一致性**。
...
- **最终一致性** 是**弱一致性的一种特例**，在这种一致性下系统保证用户最终能够读取到某操作对系统特定数据的更新（读取操作之前没有该数据的其他更新操作）。"不一致性窗口"的大小依赖于交互延迟、系统的负载，以及数据的副本数等。
...
**操作幂等性** 幂等性，其实是一个数学概念。幂等函数，或幂等方法，是指可以使用相同参数重复执行，并能获得相同结果的函数。幂等操作的特点是**其任意多次执行所产生的影响均与一次执行的影响相同**。也就是说，**同一个方法，使用同样的参数，调用多次产生的业务结果与调用一次产生的业务结果相同**。

之所以需要操作幂等性，是因为为了**保证数据的最终一致性**，很多事务协议都会有很多重试的操作，如果一个方法不保证幂等，那么将无法被重试。**幂等操作的实现方式有多种，如在系统中缓存所有的请求与处理结果、检测到重复操作后，直接返回上一次的处理结果等**。
...
- **本地消息表**
  - 创建订单时，将减少库存消息加入再本地事务中，一起提交到数据库存入本地消息表，然后调用库存系统，如果成功则修改本地消息状态为成功，如果调用库存系统失败，则由后台定时从本地消息表中取出未成功的消息，重试调用库存系统。
- **消息队列**：目前RocketMQ中支持事务消息，它的工作原理是：
  ...
  - 如果消费失败，则根据重试策略进行重试，最后还失败则进入**死信队列**等待进一步处理。
- **Seata**
  - 阿里开源的分布式事务框架，支持AT、TCC等多种模式，底层都是基于两阶段提交理论来实现的。
- **两阶段提交/三阶段提交（2PC/3PC）**

![两阶段提交](C:\Users\pc\Desktop\知识累积\两阶段提交.png)

- **TCC**

![TCC](C:\Users\pc\Desktop\知识累积\TCC.png)
...
# 二十六、[消息队列](https://www.zhihu.com/question/54152397)
...
![img](https://pic1.zhimg.com/80/v2-8234b69f5e46550bc5d95db96251d8b6_720w.webp?source=1940ef5c)
...
# 二十七、[ZooKeeper](https://zhuanlan.zhihu.com/p/45728390)
...
![img](https://pic4.zhimg.com/80/v2-2d20f7c225ea3e8e46e59c8510a9ad1f_720w.webp)
...
![img](https://pic1.zhimg.com/80/v2-82aaec49bd5e7ac462fd2b4b743114bc_720w.webp)
...
![img](https://pic1.zhimg.com/80/v2-74dae9e579686731c1f9c6882bd0e8cc_720w.webp)
...
- [x] 17.docker出错如何排查？（看日志）

- [x] 18.高并发场景下，如何保证程序数据保存到redis的线程安全问题？（**redis是单线程的，非常安全，redis6之后是多线程的，但是事务提交还是由一个线程执行，很安全**，同时redis的数据一致性问题，参考redis章的三个解决办法）
...
- [x] 23. 进程运行时，cpu会干什么？![img](https://img-blog.csdn.net/20180930105650409?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2h1bnRlcl9fXw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)

  （最开始由**操作系统将程序加载到内存空间**，操作系统负责**代码段、数据段和bss段**的加载，并在内存中为这些段分配内存空间，栈也由操作系统分配，而堆是程序员自己显式的申请和释放的（一般说的数据段为：数据段+BSS+堆）

  ​	然后cpu开始登场，

  ​	读取**程序计数器**内的一条内存上的指令地址，并存放到指令寄存器

  ​	分析**指令寄存器**中的指令，如果是计算类型指令，则分配给**逻辑运算单元**，若果是存储类型指令，则分配给**控制单元**执行

  ​	最后程序计数器自增，开始读取下一条指令。

  ​	）

- [x] 24.为什么重写equals方法，就必须要重写hashcode方法？

​		首先equals方法是比较对象的地址是否相同，如果相同则返回true，即优先采用==进行比较

​		其次，hashcode主要存在于对象头中，用于存放到散列表时，会直接调用当前的对象的Native 的hashcode方法，拿出hashcode取模进行运算的，所以两个相同的对象的hashcode一定相等才行，而也可能存在hashcode相等而对象不相等的情况，这就是所谓的hash冲突，所以我们重写equals方法在这里是不能保证我们存入散列表时，被认定为是同一个对象，因为**散列表根本优先不使用equals方法判断是否是同一个对象，而是用hashcode方法，只有当判断存在当前对象时，才会调用equals方法与当前对象做对比（这里是为了避免hash冲突），并进行覆盖**）

​		所以，当我们从写equals方法时，我们如果不从写hashcode方法，会造成在将对象存放到散列表时，两个相同的对象却拥有不同的hashcode而被分到不同的位置区域（相同的对象存在两份），再我们想要取出对象时，就会出现一些难以预料的错误。

- [x] 25.手动解析http传递的json请求

​		（初步实现使用socket和serverSocket，来接收http请求，并用byte[] 接收内容，正则表达式/json解析包来解析json请求）

- [x] 26.mysql执行命令的过程

​	（https://xiaolincoding.com/mysql/base/how_select.html#%E7%AC%AC%E4%B8%80%E6%AD%A5-%E8%BF%9E%E6%8E%A5%E5%99%A8）

​	（**建立连接**、**查询缓存**（如果是select，看上次清空之前是否执行过这个命令）、**解析sql**（词法分析，语法分析）、**执行sql**（

- prepare 阶段，也就是预处理阶段；
- optimize 阶段，也就是优化阶段；
- execute 阶段，也就是执行阶段：主键索引查询、全表扫描、索引下推；））

​	（注：这里执行sql时的三阶段和redis事务的三阶段不同（MULTI开启、QUEUED入队、EXEC执行））
...
class Solution:
    ...
    def minMovesToMakePalindrome(self, s: str) -> int:
        ...
        for i in range(mid):
            j = r
            # 找到右边与当前枚举的字母匹配的字母
            while j > i and s[j] != s[i]: j -= 1
            # i = j表示没有找到说明当前的字符串的长度为奇数
            if i == j:
                odd = i
            else:
                # 说明找到了与之匹配的字母
...
- redis的set扩展命令（set ex px nx）

- redisson（看门狗）

- redlock+redisson:集群+看门狗
...
- [x] 30.mysql[索引失效场景](https://segmentfault.com/a/1190000021464570)

  (主要弄懂索引的原理是数据的排序，就知道索引为什么失效了)

- [x] 31.wait_time出现在什么地方？

​	（请求结束的一方，在二次接收到另一方的返回时，过2MSL后关闭连接。）

- [x] 32.服务器wait_time过多？（**结合40**）（大量短链接运行后）
...
- [ ] 34.[oom如何排查](https://www.cnblogs.com/jelly12345/p/15007745.html)？
  ...
  ⑧代码优化，根据压测的情况去进行一定的jvm参数的调优

- 一般生产系统都会设置系统发生OOM时，生成当时的dump文件（-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/usr/local/base)
...
- `signal()`会唤醒某个等待线程；
- `signalAll()`会唤醒所有等待线程；
- [x] 40.服务器端出现close_wait过多，是什么原因？（**结合32**）

close_wait过多，要么表明你的应用程序写的有问题，**没有合适的关闭socket**(因为close_wait需要socket关闭才能解除)；要么是，你的服务器cpu处理不过来（**cpu太忙**）或者你的应用程序一直睡眠到其它地方（锁，或者文件I/O等等），你的应用程序获得不到合适的调度时间，造成你的程序**没法真正的执行close操作**。

**其解决方案就是：检查代码！！！**

- [x] 41.什么是短链接，什么是长连接

​	长连接：长连接，指在一个连接上可以**连续发送多个数据包**，在连接保持期间，**如果没有数据包发送，需要双方发链路检测包**。

​	短连接：短连接（short connnection）是相对于长连接而言的概念，指的是在数据传送过程中**，只在需要发送数据时，才去建立一个连接**，数据发送完成后，则断开此连接，即每次连接只完成一项业务的发送。

42.长连接、短链接、keep-alive的区别

![img](https://files.mdnice.com/user/4236/5d5a1d67-6c9a-4cba-8ed7-7da7302b7ae4.png)

- [x] 43.socket失去连接对read的影响？

1 ,提前正常关闭连接(close())，另一方 read() 返回-1，不阻塞。

2, 非正常关闭连接(网络断掉,线程挂掉)，另一方read() 抛异常,不阻塞.

3，另外,如果没有read(), 另一方会导致write()阻塞。
4，socket关闭连接，一方在write(),会抛异常。

**（简单点说，正常端开连接，read时不会阻塞，write会阻塞，而非正常断开时，read和write都会抛异常）**

- [x] 44.499状态码是什么？

  （出现在客户端访问请求未等到服务端处理就主动断开，而服务端处理就会返回499用于记录错误）

- [x] 45.[Spring事务什么时候会失效](https://zhuanlan.zhihu.com/p/101396825)？（**主要看是不是代理对象在执行，因为底层实现是AOP**）

- @Transactional修饰的不是pubilc方法

- 数据库不支持事务（MyISAM就不支持事务）

- 没有被Spring管理（如把@Component、@Service、@Controller去掉）

- 自身调用自己

- 数据源没有配置数据管理器

- 不支持事务，如@Transactional(propagation = Propagation.NOT_SUPPORTED)

- 异常被吃了，不被抛出（把异常吃了，然后又不抛出来，事务怎么回滚吧！）

- 异常抛出类型错误（默认是RuntimeException，可以通过@Transactional注解添加rollbackFor修改）

- [x] 46.CGLIB是什么？

  简单点说，CGLIB就是一个动态代理的jar包，比jdk更高效
...
- [x] 47.什么是雪花算法？
  ...
  - ![img](https://pic4.zhimg.com/80/v2-0ca4a4125b1cbda69cfa972b1e568ffb_720w.webp)
...
从图片可以看出，负载均衡是处于客户端和服务器之间，用户访问负载均衡器，再由负载均衡器将请求转发给后端服务器。
...
[Niginx](https://zhuanlan.zhihu.com/p/364588916):*Nginx* (engine x) 是一个高性能的[HTTP](https://link.zhihu.com/?target=https%3A//baike.baidu.com/item/HTTP)和[反向代理](https://link.zhihu.com/?target=https%3A//baike.baidu.com/item/%E5%8F%8D%E5%90%91%E4%BB%A3%E7%90%86/7793488)web服务器
...
https://www.bilibili.com/video/BV13W4y1E7R6?p=63&spm_id_from=pageDriver&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b

- [x] 53.[消息队列是什么](https://www.zhihu.com/question/54152397)？有哪些类型？

  消息队列简单理解为，**把传递的消息放在队列里**

  （kafka,RabbitMQ,RocketMQ）

- [x] 54.什么是[分布式事务](https://juejin.cn/post/6844903734753886216)，有哪些实现方案？

首先我们了解下**什么是事务**，**事务**可以理解为“**要么什么都不做，要么就做全套**”的机制。例如数据库的**ACID**。
...
- 本地消息表
- 消息队列
- Seata：阿里的开源框架，底层是采用两阶段提交理论来实现的。



- [x] 55.什么是服务熔断，什么是服务降级，区别？

- 服务熔断：当服务A调用服务B不可用时，上游服务A为了保证自己不受影响，从而不再调用服务B，直接返回一个结果，减轻服务A和服务B的压力，直到服务B恢复。
- 服务降级：当发现压力过载时，可以通过关闭某个服务，或限流某个服务来减轻系统压力，则就是服务降级。
- 相同点:
  - 都是为了防止系统崩溃
  - 都让用户体验到某些功能暂时不可用
- 区别：熔断是下游服务故障触发的，降级是为了降低系统负载。

- [x] 56.数据一致性模型有哪些？

- 强一致性：每一个操作必须保证一致性
- 弱一致性：数据一致性需要一段时间同步
- 最终一致性：弱一致性的特例，保证数据最终能一致

- [ ] 57.[ConcurrentHashMap的扩容机制](https://www.bilibili.com/video/BV13W4y1E7R6/?p=87&spm_id_from=pageDriver&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)

- ConcurrentHashMap是一个线程安全的hashMap，与hashMap使用无异。
- 要区分版本1.7，1.8
- 1.7 ：每一个小区间（）扩容，然后转移元素，segment指向新的扩容后的小map
- 1.8 ：大区间扩容，多线程来转移元素

- [x] 58.AQS是什么？

java的线程同步队列，时jdk中很多锁的核心实现框架

关键词：信号量state，线程组成的双向链表队列

- [x] 59.什么是CAP？

![CAP](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/12/10/1679817cdb4305db~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
- [ ] 62.[Zookeeper是什么](https://zhuanlan.zhihu.com/p/45728390)？
...
指的是where后面的查询条件，有无索引里的做左边的索引列，与顺序无关，如联合索引是（a,b,c）而查询条件是（b,a,c）那么是符合索引的，而（b,c）就不会走索引，因为缺失了最左的a索引列（归根到底还是跟innodb的索引实现的B+树的排序有关）

- [x] 64.[Spring是如何解决循环依赖的](https://www.bilibili.com/video/BV1yG411p752/?spm_id_from=333.337.search-card.all.click&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)？

**三级缓存**

详细解答：https://cloud.tencent.com/developer/article/1497692

<img src="C:\Users\pc\Desktop\知识累积\三级缓存的流程.png" alt="三级缓存" style="zoom: 67%;" />
...
- [x] 66.负载均衡方案有哪些？
  ...
  - **软件方案**：硬件负载均衡价格昂贵，在实际应用中远不如**软件负载均衡普遍**。常用的软件负载均衡软件有**Nginx、LVS、HaProxy、ats、perlbal、pound**等。
...
排除其它因素，如果客户端和服务器端直接通过一个路由器连接，但带宽只有10Kbps，却同时有多个应用需要传输远超带宽的数据量200Kbps，这时候会造成大量数据丢失，从而表现为响应延时。

- **处理带宽不够**

排除其它因素，如果客户端和服务器端直接通过一个路由器连接，且带宽足够，但服务器端处理能力不足，也会造成响应延时，可以将Windows电脑中的定位功能关闭掉来降低延迟。
...
微服务架构（通常简称微服务）是指开发应用所用的一种架构形式。通过微服务，可将大型应用分解成多个独立的组件，其中每个组件都有各自的责任领域。在处理一个用户请求时，基于微服务的应用可能会调用许多内部微服务来共同生成其响应。

- [x] 74.[SQL慢查询是什么](https://tech.meituan.com/2022/04/21/slow-query-optimized-advice-driven-by-cost-model.html)？

​	慢查询是指数据库中查询时间超过指定阈值（美团设置为100ms）的SQL

- [x] 75.SQL慢查询优化

  [视频链接](https://www.bilibili.com/video/BV1cD4y1k7Fy/?p=7&spm_id_from=pageDriver&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)

  - 是否走了索引
  - 是否是最优索引
  - 是否有多于查询
  - 是不是数据过多，可考虑分库分表、读写分离
  - 机器性能是不是太低

- [x] 76.[Innodb如何实现事务](https://www.bilibili.com/video/BV1cD4y1k7Fy/?p=10&spm_id_from=pageDriver&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)？

  - 先读取数据所在页到buffer pool
  - 再修改buffer pool中的数据
  - 再生成一个RedoLog对象，并存入LogBuffer中
  - 再针对语句生成UndoLog日志，用于事务回滚
  - 如果事务提交（commit）那么就持久化RedoLog对象，后续还有其他机制将Buffer pool中修改的数据持久化到磁盘
  - 如果事务回滚，则利用UndoLog回滚

- [ ] 77.[如何分库分表](https://www.bilibili.com/video/BV1Qv4y1w72q/?spm_id_from=pageDriver&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)？

- 前提：
  - 分库分表是最终解决方案
  - 优先采用增添索引、优化索引、优化sql语句、读写分离、增添从库的操作，最终方案才是分库分表
- 过程：
  - 分库
  - 分表：横向拆分，纵向拆分
  - 分库+分表
...
![分库分表](C:\Users\pc\Desktop\知识累积\分库分表.png)

[分库分表带来的问题](https://www.bilibili.com/video/BV1aY411K7qy/?spm_id_from=pageDriver&vd_source=cb5cce1bdc5ab99fce812684e0c31a2b)：
...
这些方法基本都很明确通用，比如Netflix的Hystrix：[https://github.com/Netflix/Hyst](https://link.zhihu.com/?target=https%3A//github.com/Netflix/Hystrix)

- [x] 79.什么是服务熔断，什么是服务降级，两者的区别？

- 服务熔断：当服务A调用的某个服务B不可用时，上游服务A为了保证自己不受影响，从而不在调用服务B，直接返回一个结果，减轻服务A和服务B的压力，直到服务B恢复。

- 服务降级：当发现系统压力过载时，可以通过关闭某个服务，或限流某个服务来减轻系统压力，这就是服务降级。
  - 相同点：
    - 都是为了防止系统崩溃
    - 都让用户体验到某些功能暂时不可用
  - 不同点：
...
- [x] 80.什么是服务雪崩，什么是服务限流？

- 服务雪崩：当服务A调用服务B，服务B调用服务C，此时大量请求突然请求服务A，假如服务A本身能抗住这些请求，但是如果服务C抗不住，导致服务C请求堆积，从而服务B请求堆积，从而服务A不可用，这就是服务雪崩，解决方式就是服务熔断和服务降级。

- 服务限流：再高并发请求下，为了保护系统，可以对访问服务的请求进行数量上的限制，从而防止系统不被大量请求压垮，在秒杀中，限流非常重要。

- [x] 81.cookie,session,token的区别？
...
- [x] 83.https如何实现密钥交互，数字认证，已经数字认证流程？

(详看https章节)

- [x] 84.mysql慢查询，如何客观衡量原因（提示command）？

https://landybird.github.io/mysql/2019/09/23/MYSQL%E6%85%A2%E6%9F%A5%E8%AF%A2%E4%BC%98%E5%8C%96/

- [x] 85.如何设计目录？

  https://www.cnblogs.com/lvyahui/p/6439287.html
...
- [ ] 87.redis底层数据结构，以及为什么用dict+skipset，以及查询时间复杂度？

- [x] 88.时间片轮转算法的优缺点？

- 优点：定时有相应（**雨露均沾**）
- 缺点：时间片过长**退化**为FCFS；**切换上下文较多**

- [ ] 89.mysql单表数量最大建议是多少（提示B+树高）？
- [x] 90.[mysql的Limit变慢的原因](https://www.jb51.net/article/199889.htm)？
...
- [ ] 91.redis的分布式锁的问题？（锁失效但未结束，已结束但锁被别的抢占了，释放错了）
...
- 协程：协程的调度完全由用户控制（也就是在用户态执行）。协程拥有自己的寄存器上下文和栈。协程调度切换时，将寄存器上下文和栈保存到线程的堆区，在切回来的时候，恢复先前保存的寄存器上下文和栈，直接操作栈则基本没有内核切换的开销，可以不加锁的访问全局变量，所以上下文的切换非常快。（**最主要的区别时用户自己调度**）
...
- [x] 93.[如何设计百万级并发项目](https://juejin.cn/post/6990307911117307934)？
  ...
  <img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5c108b6b84474b80bb25336d4a15988f~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp" alt="图片" style="zoom:50%;" />

- [ ] 94.计算机网络，虚拟地址是什么？

- [x] 95.mac地址和IP地址什么区别，以及两者什么时候使用？

MAC地址是烧录在网卡或者接口上的**物理地址**，具有二层意义和**全球唯一性**，一般不能被改变。IP地址是网络中的主机或者三层接口在网络中的**逻辑地址**，在**同一个网络内具有唯一性**。

- 在网络层及以上使用IP地址
- 在数据链路层及物理层使用MAC地址
- 使用IP地址的IP数据报一旦交给了数据链路层，就被封装成了MAC帧，MAC帧在传说时使用的时源地址和目的地址都是MAC地址，这两个MAC地址放在MAC帧首部。
- 连接在通信链路上的设备（主机或路由器）在收到MAC帧时，根据MAC帧首部的MAC地址决定收下还是丢弃，只有在剥去MAC帧首部和尾部后把MAC层的数据上交到网络层，网络层才能在IP数据报的首部中找到源IP地址和目的IP地址。
- ARP协议：根据IP地址解析出MAC地址（网络层协议）
- [x] 96.[浮点数的存储形式](https://juejin.cn/post/6844903934356619271)，以及为什么0.1+0.2!=0.3，从操作系统层面讲？

**简单点说：浮点数在操作系统中存储形式类锁科学计数法，包括三部分，符号位s、指数位e、有效数位f，通过公式 (-1)^s \* 1.f \* 2^e 可计算出十进制数，但是以0.1为例，我们可以确定s = 0， f = 1只需要算出e的大小，结果我们发现e只能算出近视值，所以导致了0.1其实不是0.1而是近视值。**
...
1. 第一部分是**符号位**，用s表示，代表正负，要记住的是在浮点数的范围内，所有数字都是有符号的；
2. 第二部分是**指数位**，用e表示，代表指数，用8位bit表示的数字范围是0~255，为了同时表示大数和小数，我们把0~255去掉头尾（0，255后面会用到）的1~254去映射到-126~127，这样同时可以表示最大最小数字；
3. 第三部分是**有效数位**，用f表示，代表的是有效的数位；

综合上述表示和科学计数法，我们的浮点数就可以表示为公式

**(-1)^s \* 1.f \* 2^e**

看完公式有没有发现问题？你会发现，我们这个公式无法表示0，的确，这是一个巧妙的设计，我们用0（8个bit都为0）和255（8个bit都为1）来表示一些特殊的数值，可以认为他们2个是特殊的flag位，比如当e和f都为0的时候，我们就认为这个浮点数是0
...
- [x] 97.线程阻塞和挂起的区别？
- 阻塞是线程内部导致的，如等待IO、等待Notify等
- 挂起是由操作系统执行调度算法引起的，使得线程暂时从内存空间离开，cpu先去执行其他线程。
- [ ] 98.
